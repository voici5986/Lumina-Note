  ...
  return (
    <div className="p-4 space-y-4 text-sm">
      ...
      <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg space-y-2">
        <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">当前任务</div>
        <div className="flex justify-between">
          ...
          <span className="font-mono">
            ...
            {taskStats.toolFailures > 0 && (
              <span className="text-red-500 ml-1">({taskStats.toolFailures} 失败)</span>
            )}
          </span>
        </div>
        <ProgressBar value={currentSuccessRate} />
        <div className="text-xs text-gray-500 text-right">
          成功率: {currentSuccessRate.toFixed(1)}%
        </div>
      </div>
      
      {/* 累计统计 */}
      <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg space-y-3">
        <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">累计统计</div>
        ...
        <div className="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-1">
            ...
            <span>完成: {taskStats.completedTasks}</span>
          </div>
          <div className="flex items-center gap-1">
            <XCircle className="w-3 h-3 text-red-500" />
            <span>失败: {taskStats.failedTasks}</span>
          </div>
        </div>
        
        {/* Token 使用 */}
        <div className="pt-2 border-t border-gray-200 dark:border-gray-700 text-gray-500">
          Token: <span className="font-mono">{totalTokensUsed.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
