...
- ✅ **设置面板**：中央弹窗式设置界面
- ✅ **主题系统**：15 套官方主题（默认/海洋/森林/薰衣草/玫瑰/落日/薄荷/靛青/拿铁/极光/极简）
...
##### 🔧 PDF 交互模式 Rust 移植（优先级：⭐⭐⭐⭐⭐ 必做）

```
目标：将 PDF 交互模式从 Python Flask 服务移植到 Rust Tauri 后端
原因：Build 版本无法依赖 Python 环境，必须打包进应用
当前状态：Python 版本（simple_pdf_server.py）依赖 PyMuPDF
```

**当前 Python 实现**：
- Flask 服务 + PyMuPDF (fitz)
- 提取 PDF 文本块位置
- 用户需要手动启动 `python simple_pdf_server.py`

**Rust 移植方案**：

| 任务 | 说明 | Crate |
|------|------|-------|
| PDF 解析 | 提取文本块位置 | `pdf-extract` 或 `lopdf` |
| Tauri Command | 暴露给前端调用 | Tauri invoke |
| 坐标映射 | 与前端 react-pdf 对齐 | 自研 |

**实现步骤**：

| 阶段 | 内容 | 时间 |
|------|------|------|
| 1. Rust PDF 解析 | 使用 `pdf-extract` 提取文本位置 | 2 天 |
| 2. Tauri Command | `parse_pdf_elements` 命令 | 1 天 |
| 3. 前端对接 | 替换 Python API 调用 | 1 天 |
| 4. 测试优化 | 坐标精度、性能优化 | 1 天 |

**预期效果**：
- ✅ Build 版本无需 Python 依赖
- ✅ 用户无需额外配置
- ✅ 性能更好（Rust vs Python）
- ✅ 应用体积更小
...
| 软件 | 类型 | 技术栈 | 价格 | 数据存储 |
|------|------|--------|------|----------|
| **Lumina Note** | 笔记 + AI | Tauri/Rust | 免费开源 | 本地 Markdown |
| **Obsidian** | 笔记 | Electron | 免费/付费同步 | 本地 Markdown |
| **Notion** | 笔记 + 数据库 | Electron | 免费/付费 | 云端 |
| **Logseq** | 大纲笔记 | Electron | 免费开源 | 本地 Markdown |
| **Roam Research** | 大纲笔记 | Web | $15/月 | 云端 |
| **Zotero** | 文献管理 | XULRunner | 免费开源 | 本地 SQLite |

### 核心功能对比

| 功能 | Lumina Note | Obsidian | Notion | Logseq | Zotero |
|------|:-----------:|:--------:|:------:|:------:|:------:|
| **编辑器** |
| Markdown 编辑 | ✅ | ✅ | ⚠️ 有限 | ✅ | ❌ |
| 实时预览 | ✅ | ✅ | ✅ | ✅ | ❌ |
...
| 优点 | 缺点 |
|------|------|
| ✅ **原生 AI Agent** - 无需插件 | ❌ 无移动端 |
| ✅ **8 个 LLM 提供商** | ❌ 无插件生态 |
| ✅ **RAG 语义搜索** | ❌ 社区较小 |
| ✅ **轻量**（Tauri 比 Electron 小 10x） | ❌ PDF 标注待开发 |
| ✅ **完全开源免费** | |
| ✅ **数据本地 + 隐私安全** | |
...
| 场景 | 推荐软件 |
|------|----------|
| **AI 辅助写作/研究** | 🥇 **Lumina Note** |
| **学术文献管理** | 🥇 Zotero |
| **团队协作** | 🥇 Notion |
| **个人知识库（重度）** | 🥇 Obsidian |
| **大纲式思考** | 🥇 Logseq |
| **隐私敏感 + AI 需求** | 🥇 **Lumina Note** |
| **轻量 + 快速** | 🥇 **Lumina Note** |
...
| 优势 | 说明 |
|------|------|
| **原生 AI** | 8 个模型提供商、Agent 系统、RAG、实时编辑预览 |
| **轻量技术栈** | Tauri v2 + Rust，比 Electron 更小更快 |
| **本地优先** | 数据完全本地，隐私安全 |
| **完全开源** | Apache 2.0 许可，自由修改分发 |
